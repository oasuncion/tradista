...
public class DailyPnlBusinessDelegate {
	...
	public Set<DailyPnl> getDailyPnlsByDefinitionIdCalendarAndValueDates(long positionDefinitionId,
			String calendarCode, LocalDate valueDateFrom, LocalDate valueDateTo) throws TradistaBusinessException {
		if (valueDateFrom != null && valueDateTo != null) {
			if (valueDateTo.isBefore(valueDateFrom)) {
				throw new TradistaBusinessException(String.format("'To' value date cannot be before 'From' value date.%n"));
			}
		}
		return SecurityUtil
