package finance.tradista.ir.ircollar.transfer;
...
public class IRCollarTransferManager extends IRCapFloorCollarTransferManager {

	protected ConfigurationBusinessDelegate configurationBusinessDelegate;

	public IRCollarTransferManager() {
	...
	public void fixCashTransfer(CashTransfer transfer, long quoteSetId) throws TradistaBusinessException {
		...
		if (ir == null) {
			...
			String errorMsg = String.format(
					"Transfer %n cannot be fixed. Impossible to get the %s index closing value as of %d in QuoteSet %s.",
		...
		if (!(ir.compareTo(trade.getCapStrike()) > 0) && !(ir.compareTo(trade.getFloorStrike()) < 0)) {
			// No transfer
