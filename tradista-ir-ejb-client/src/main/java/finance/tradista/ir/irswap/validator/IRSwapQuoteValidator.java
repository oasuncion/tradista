	...
	public void validateQuoteName(String quoteName)
		...
		if (data.length < 3) {
			throw new TradistaBusinessException(
					String.format(
							"The quote name (%s) must be as follows: %s.Index.Maturity%n",
							quoteName, IRSwapTrade.IR_SWAP));
		}
		...
		if (new IndexBusinessDelegate().getIndexByName(data[1]) == null) {
			errMsg.append(String.format(
					"The Index (%s) must exist in the system.%n.",
					data[1]));
		}
		if (Tenor.valueOf(data[2]) == null) {
			errMsg.append(String.format(
					"The swap maturity (%s) must be a valid Tenor: %s%n.",
					data[2], Tenor.values()));
		}
