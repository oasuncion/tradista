	...
	public void validateQuoteName(String quoteName)
		...
		if (data.length < 3) {
			throw new TradistaBusinessException(
					String.format(
							"The quote name (%s) must be as follows: (%s).Isin.ExchangeCode%n",
							quoteName, Bond.BOND));
		}
		...
		Bond bond = new BondBusinessDelegate().getBondByIsinAndExchangeCode(
				data[1], data[2]);
		if (bond == null) {
			errMsg.append(String
					.format("The bond %s in the exchange %s must exist in the system.%n",
							data[1], data[2]));
		}
		if (errMsg.length() > 0) {
