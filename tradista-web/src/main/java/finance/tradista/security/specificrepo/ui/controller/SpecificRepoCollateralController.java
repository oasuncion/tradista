...
@Named
@ViewScoped
public class SpecificRepoCollateralController implements Serializable {

	private static final long serialVersionUID = 7445438652264271942L;
	...
	public void refresh(long tradeId) {
		...
		try {
			...
			if (trade != null) {
				...
				Security repoSecurity = trade.getSecurity();
				...
				allocatedSecurities = specificRepoTradeBusinessDelegate.getAllocatedCollateral(trade);
				...
				for (Book b : configuredBooks) {
					inventory = productInventoryBusinessDelegate.getProductInventories(LocalDate.now(), LocalDate.now(),
							repoSecurity.getProductType(), 0, b.getId(), false);
	...
	public void refreshDonutModel() {
		...
		try {
			BigDecimal collateralMarketValue = specificRepoPricerBusinessDelegate
					.getCurrentCollateralMarketToMarket(trade);
